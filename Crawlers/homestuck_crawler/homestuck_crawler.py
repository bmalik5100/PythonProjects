...
def getConversationSoup(body):
    ...
    if "o_chat-log" in body:
        chatarea = soup.body.find("p", attrs={"class" : "o_chat-log"}).findChildren()
        users = set()
        convo = list()
        
        for span in filter(lambda t : t.name == "span" and ':' in t.text, chatarea):
            log = span.text.replace('\n', ' ').split(":")
            users.add(log[0].strip())
            convo.append({ "user" : log[0].strip(), "message" : log[1].strip() })

        if len(users) != 2:
            return list(), set()
        else:
            #print(f"conversation logs for {PAGE}: {len(convo)}\tUsers: {users}")
            return convo, users
    else:
        return list(), set()
...
if __name__ == "__main__":
    ...
    while running and PAGE <= MAXPAGE:
        ...
        try:
            ...
            logs, users = getConversationSoup(html.content.decode())
    ...
    with open("/home/malik/Documents/Scripts/Python/homestuck_crawler/test.html", "r") as f:
       testbody = f.read()
    logs, users = getConversationSoup(testbody)
    users = list(users)
